...
def one_subject_to_dicom(datadir: str, save_to:str=None, mb:master_bar=None) -> None:
    ...
    for path in os.listdir(datadir):
        if path.startswith("HEAD_MRSI-DKI"):
            dki_datadir = os.path.join(datadir, path)
            break

    # Check if DKI data exist
    if dki_datadir is None:
        raise ValueError(f"HEAD_MRSI-DKI folder not present in `{datadir}`")
...
def to_dicom(datadir:str, save_to:str=None) -> None:
    ...
    Returns: None
    """
    subjects_list = os.listdir(datadir)
...
# Step 1 command: python dkip.py to-dicom -dd .\sample_data\ -st sample_dicoms_data

# Step 2: Run `multi_subject_dke` for all subjects DKE processing
# Step 2 command: python dkip.py multi-subject-dke -pf DKEParameters.dat -dd sample_dicoms_data --delete_ima
=======
import os
import logging
import shutil
...
>>>>>>> be8b3272eab8b292be5b4a3376a686f2bdc9af8b
